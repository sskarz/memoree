...
How each of the six supported coding assistants is wired to Memoree: the distribution channel, the installation path, the capture and recall mechanism, and the notable behavioral differences between agents.
...
The Memoree answer is the same in every case: map the assistant's native lifecycle events onto the shared `capture.ts` and recall logic in `src/`, emit the same row format to Memoree, and confine all per-agent differences to the thin shims under `src/hooks/{codex,cursor,hermes,pi}/`. Adding a new assistant means writing a new shim, not a new memory engine.
...
|---|---|---|---|---|---|
| Claude Code | Marketplace plugin | `~/.claude/plugins/memoree/` | Claude Code Marketplace | Model-only (`additionalContext`) | Reference implementation. Full hook set. Rules and goals injected every SessionStart. |
| Codex | `~/.codex/hooks.json` | `~/.codex/hooks.json` + bundle | npm (`memoree`) | User-visible (`hook context: <text>` in TUI) | Async setup spawned as a detached process because Codex has no async hook channel. Rules/goals block deliberately omitted to avoid TUI clutter. |
...
| OpenClaw | Native extension | `~/.openclaw/extensions/memoree/` | ClawHub | Model-only (via `before_prompt_build` system context) | Event-driven: `agent_end` for capture, `before_agent_start` for recall. No PreToolUse VFS intercept; exposes agent-facing `memoree_search`/`memoree_read`/`memoree_index` tools directly. |
| Hermes | Shell hooks + MCP server | `~/.hermes/skills/memoree-memory/` | npm | User-visible in TUI (same wire as `context`) | MCP tools (`memoree_search`, `memoree_read`, `memoree_index`) registered alongside the shell hooks. Recall via grep on `~/.memoree/memory/` or MCP. |
| pi | `~/.pi/agent/AGENTS.md` + TypeScript extension | `~/.pi/agent/memoree/` | npm (raw `.ts`, compiled by pi) | Depends on pi configuration | No PreToolUse intercept available. Recall via grep on `~/.memoree/memory/`. Wiki worker shelled via `pi --print`. |

---

## Claude Code: the reference implementation

Claude Code is the integration Memoree was designed around. The plugin lives at `~/.claude/plugins/memoree/` and is distributed through the Claude Code Marketplace. It wires every available lifecycle event: `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `Stop`, `SubagentStop`, and `SessionEnd`.
...
Codex's `additionalContext` is rendered as `hook context: <text>` in the user-facing TUI history cell alongside both model context and user-visible entries. Because of this, the bulky memory-usage instructions and the rules/goals block that Claude Code injects are deliberately omitted for Codex. The Codex agent discovers memory tiers via `memoree --help` and shell commands on demand.
...
Second, Cursor's PreToolUse only intercepts Shell tool calls. It cannot intercept Write or Edit on the memory path, so the goal management instructions use the CLI variant (`memoree goal add/list/...` invoked as shell commands) rather than the Write-tool-intercept pattern available in Claude Code.

---

## OpenClaw: native extension with contracted tools

OpenClaw loads plugins from `~/.openclaw/extensions/memoree/` as a native extension using a synchronous `register(pluginApi)` entry point defined in `harnesses/openclaw/src/index.ts`. The `register` function must return synchronously; all async work executes inside a fire-and-forget IIFE.
...
OpenClaw has no PreToolUse analog. Instead, the extension registers three agent-facing tools (`memoree_search`, `memoree_read`, `memoree_index`) plus two write tools (`memoree_goal_add`, `memoree_kpi_add`) via `pluginApi.registerTool`. The SKILL.md body embedded at build time (`__MEMOREE_SKILL__` constant) instructs the agent to call `memoree_search` before answering questions about past work. OpenClaw also registers a `MemoryCorpusSupplement` so other OpenClaw plugins that expose a `memory_search` tool can federate queries into Memoree automatically.

Because OpenClaw's bundle scanner treats any `process.env` access in a file that also calls `fetch()` as `env-harvesting`, all `MEMOREE_*` environment reads are rewritten by esbuild's `define` to `globalThis.__memoree_tuning__?.MEMOREE_X`, and `applyOpenclawTuning` bridges the user's `openclaw.json` plugin config into that global.

---

## Hermes: shell hooks plus MCP

Hermes wires Memoree through a skill directory at `~/.hermes/skills/memoree-memory/` that contains both shell hook scripts and an MCP server registration. The shell hooks fire on Hermes's event names and relay context via `{ context: "..." }` on stdout. The MCP server (`src/mcp/server.ts`) is spawned as a stdio subprocess and exposes `memoree_search`, `memoree_read`, and `memoree_index` as MCP tools.
...
pi loads the Memoree extension from `~/.pi/agent/memoree/` as raw TypeScript files that pi compiles at load time. There is no pre-compiled bundle; the extension source ships in `harnesses/pi/` under version control. A static `AGENTS.md` block at `~/.pi/agent/AGENTS.md` provides recall instructions that pi injects into every session's system context.

pi has no PreToolUse hook. Recall happens via grep on `~/.memoree/memory/`, as documented in the `AGENTS.md` block. The wiki worker (`src/hooks/pi/wiki-worker.ts`) is invoked by the extension and shells out to `pi --print --provider <p> --model <m>` for summary generation, using pi's non-interactive mode so it does not recurse back into the extension.
...
- Capture is gated by `MEMOREE_CAPTURE !== "false"`. When that flag is set, the hook runs read-only: no DDL, no INSERTs.
...
- All writes use the immutable version-bumped pattern for rules, skills, goals, and KPIs to avoid Memoree's UPDATE-coalescing quirk.
