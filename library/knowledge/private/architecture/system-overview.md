...
How Memoree is laid out as a monorepo, the major subsystems, and how a shared core fans out into six per-agent integrations backed by a single Memoree substrate.
...
│   └── cli/                ← unified `memoree install` CLI + per-agent installers
...
└── bundle/                 ← unified `memoree` CLI build output
```
...
flowchart TB
    ...
    subgraph core["Shared core (src/)"]
        ...
        apiClient["Memoree API client"]
    end

    subgraph memoree["Memoree substrate"]
...
**Memoree API client.** `src/memoree-api.ts` is the single chokepoint to storage. It owns table creation, lazy schema healing, and query execution; `src/storage/schema.ts` is the single source of truth for every column.
