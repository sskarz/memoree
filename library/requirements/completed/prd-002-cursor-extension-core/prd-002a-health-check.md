...
This sub-feature is the extension's "is everything in place?" engine. It answers, continuously and on demand, whether the local environment can actually run Memoree in Cursor, and it removes the single biggest source of setup friction: hand-editing `~/.cursor/hooks.json`. Concretely it does two jobs. First, the **health check** detects whether the `memoree` and `cursor-agent` CLIs are installed and resolvable on `PATH`, whether their versions are sane, and whether `cursor-agent` is logged in. Second, the **auto-wiring** step writes the seven Memoree lifecycle hooks into `~/.cursor/hooks.json` for the developer so they never have to run a terminal command or paste JSON.
...
- Detect, without developer action, whether `memoree` is installed and resolvable, and report a clear present/absent state.
- Detect whether `cursor-agent` is installed and resolvable, and whether it is logged in, and report each independently.
- Auto-wire all seven Memoree lifecycle hooks into `~/.cursor/hooks.json` on the developer's behalf, idempotently, preserving any non-Memoree hooks already present.
...
- **Installing the `memoree` CLI automatically.** This sub-feature detects and guides; whether the extension may run a global `npm i` is an open question owned by the index PRD. Default posture: detect-and-guide.
- **Authenticating Memoree itself.** the removed cloud sign-in command state and the login flow belong to [`prd-002b-auth-secrets`](./prd-002b-auth-secrets.md). This sub-feature only *reads* login state to compose overall health. (`cursor-agent` login is a prerequisite check here; *Memoree* login is 002b.)
...
The health check produces a structured result over four independent dimensions. Independence matters: a developer can be logged into Memoree but missing `cursor-agent`, and the status must say so precisely.

| Dimension | Question | How it is determined | If it fails, the developer is told |
|---|---|---|---|
| **D1: `memoree` CLI** | Is the `memoree` CLI installed and resolvable? | PATH resolution (the same `which`/`where` strategy as `src/utils/resolve-cli-bin.ts:29-51`), then a version probe. | "Memoree CLI not found. Install it to enable shared memory," with a copyable command and docs link. |
...
| **D4: Hooks wired & current** | Are the seven Memoree hooks present in `~/.cursor/hooks.json` and matching the current bundle version? | Read `~/.cursor/hooks.json`, match Memoree entries via the existing `isMemoreeEntry` shape (`src/cli/install-cursor.ts:62-71`), compare against the version stamp. | "Hooks not wired" or "Hooks out of date" with a one-click "Wire / Refresh" action. |
...
2. **Preserve foreign hooks.** Any hook entry that is not a Memoree entry must survive untouched. Reuse the merge semantics that filter on `isMemoreeEntry` and re-append (`src/cli/install-cursor.ts:73-85`).
...
4. **Bundle presence is a precondition.** Wiring assumes the bundle exists at `~/.cursor/memoree/bundle/`. If it is absent, wiring must report the missing bundle rather than writing dangling `node "...bundle/..."` commands.
...
|---|---|---|
| D1 missing | Memoree CLI not found on PATH. | Copy install command + open docs. |
...
| Bundle absent | Hook bundle missing at `~/.cursor/memoree/bundle/`. | Re-run extension provisioning / CLI install guidance. |
...
| ID | Criterion |
|---|---|
| AC-1 | Given `memoree` is not on `PATH`, when the health check runs, then D1 reports "missing" and a copyable install command plus docs link is offered. |
...
| AC-4 | Given a `hooks.json` with no Memoree entries, when auto-wiring runs, then all seven lifecycle events are added (including the `Shell` matcher on `preToolUse`) and any pre-existing foreign hooks are preserved unchanged. |
...
| AC-7 | Given the bundle is absent at `~/.cursor/memoree/bundle/`, when auto-wiring is attempted, then it refuses to write dangling commands and reports the missing bundle. |
...
- [ ] May the extension shell out to wire hooks via the `memoree cursor install` code path, or should it own a parallel writer that imports the same canonical config to avoid drift? (Drift risk argues for sharing the config module.)
...
- [`prd-002b-auth-secrets`](./prd-002b-auth-secrets.md): owns the removed cloud sign-in command state that this check reads.
