...
2. **The change set is knowable.** Memoree already reasons about unstaged changes elsewhere: the SessionEnd auto-build hook gates rebuilds on `git diff --name-only` over source globs (`src/graph/last-build.ts:11-12`). The same diff identifies which files the developer is currently editing, and the snapshot's `source_file` field maps those files onto nodes (`src/graph/types.ts:99-100`).
