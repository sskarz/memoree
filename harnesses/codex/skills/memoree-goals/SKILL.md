...
3. Write the goal file at `~/.memoree/memory/goal/<owner>/opened/<uuid>.md` with the goal description as body.
...
1. **Find it:** `memoree goal list --mine` and match the user's reference to a `goal_id`. If ambiguous, show the candidates and ask.
...
```bash
Write the file at ~/.memoree/memory/kpi/<uuid>/<kpi-slug>.md with:
  <KPI name>
