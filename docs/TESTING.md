...
| `npx @sskarz/memoree uninstall` / `--purge` | S | — | — | Default uninstall unwires harnesses and keeps `~/.memoree` plus leftover plugin copies. `--purge --yes` deletes staged pkg, leftover plugin dirs, Memoree-managed skills, embeddings, and `~/.memoree`. Isolated-HOME install/uninstall/purge; TTY confirm defaults to No; non-TTY requires `--yes`. Does not drop PostgreSQL schemas or `memoree-runtime`. |
