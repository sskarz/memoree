...
| Antigravity PreToolUse steer (never `allow`) | S | V | — | `{ decision: "deny", reason }` on the mount; unrelated tools `{ decision: "ask" }` (agy treats `{}` as invalid_args/deny; `allow` would auto-approve) |
