...
| Claude UserPromptSubmit recall.js | S | V | L | Threshold 0.4 in live env; telemetry file uses `homedir()`. Skip `"completed"` / `"in progress"` stubs **and MCP capture digests**. Empty `project_key` rows are admitted only as a cold-start fallback when the current project has no embedded summaries. Near-tied semantic hits prefer the newest `last_update_date` so a correction outranks an older twin. Validate and live seed a stub + empty-key competitor and drive `recall.js` (not only VFS grep). |
...
| Claude SessionEnd wiki + plugin-cache-gc + graph-on-stop | S | V | L | Wiki worker uses `MEMOREE_VALIDATION_CLAUDE_HOME`. Key Facts are verified-only; unverified recs go to Next Steps; `## Corrections` is extracted first on recall. Dated technical claims get a stale-claims banner. |
...
| `index.md` / `summaries/` / `sessions/` | S | V | L | Recall, VFS grep, `ls`/`find`, exact-path reads, and index listings scope to `project_key` (git remote SHA or abs path) plus legacy empty keys. Unremappable plugin-install cwds use sentinel `__plugin_cwd__` and are not mixed into those listings. Index `description` stays `in progress` until a wiki body has `## What Happened` — never the `"completed"` stub word. MCP capture-digest summaries are omitted from `index.md`. Plugin-install cwds (`.../memoree/<semver>`, `~/.codex/memoree`) remap to `CLAUDE_PROJECT_DIR` / workspace instead of siloing sessions under the plugin version. |
...
| `memory flush` | S | — | — | |
| `sessions prune` | S | — | L | |
| `backend check` / embeddings status | S | V | L | `findMemoreeInstalls` skips Claude cache dirs with `.orphaned_at`. `embeddings status` labels the active Claude plugin version. `memoree status` / `--version` print hook stamp vs PATH CLI vs active Claude plugin. `memoree install` relinks embeddings after harness wiring. Validate/live create an unlinked `claude (9.9.9)` cache after `codex install` and require `embeddings install` to link it. |
...
| Recall injected an MCP tool-digest as Key Facts | Antigravity MCP fallback listed `memoree_read` calls as facts. | Source: digest Key Facts are a disclaimer; `isInjectableRecallHit` rejects `<!-- memoree-mcp-summary -->`; index filters those rows. |
| Wiki Key Facts listed an unverified recommendation | Ghostty `macos-titlebar-style = tabs` was recalled as fact though this session never applied it. | Source: wiki prompt Key Facts are VERIFIED only; Corrections section + recency tie-break on recall. |
| Cursor/Claude session tagged `project=0.7.153` | Hook cwd was the plugin cache version dir. | Source: `isPluginInstallCwd` + `resolveWorkspaceCwd`; unremappable plugin cwd uses sentinel `__plugin_cwd__` (not `''`, so it is not mixed into every project's index). `resolveDirConfig` remaps cwd before walking `.memoree`. |
