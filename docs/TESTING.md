...
| Antigravity capture + Stop wiki (`agy -p`) | S | V | L | `agy -p` loads hooks.json but does not execute command hooks. Unaided capture is MCP tool-call rows (`captureMcpToolCall`); MCP capture skips embedding so a hung daemon cannot delay `tools/call`, then detaches a summary+embed worker so auto-recall (summaries-only) can see the session. PostToolUse skips `memoree_*` / `call_mcp_tool` to avoid duplicate interactive rows. `waitForCapture` requires the UUID in `sessions`. Wiki summary stays best-effort (`requireSummary: false`) |
...
| `index.md` / `summaries/` / `sessions/` | S | V | L | Recall, VFS grep, and index listings scope to `project_key` (git remote SHA or abs path) plus legacy empty keys |
...
| Graph build git-only (no disk walk) | S | — | — | Non-git temp dir errors without writing a snapshot; tiny git repo still builds |
