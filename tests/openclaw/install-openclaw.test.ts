 ...
 * plugin bundle to ~/.openclaw/extensions/memoree/ AND patches
 * ~/.openclaw/openclaw.json to include "memoree" in both plugins.allow and
 ...
 * cannot fix its own visibility via /memoree_setup — the slash command
...
beforeEach(() => {
  TEMP_HOME = mkdtempSync(join(tmpdir(), "memoree-install-openclaw-test-"));
});
...
describe("installOpenclaw()", () => {
  it("copies the plugin bundle to ~/.openclaw/extensions/memoree/", async () => {
    writeConfig({ tools: { alsoAllow: ["memoree"] } });
    const { installOpenclaw } = await loadInstaller();
    installOpenclaw();

    const pluginDir = join(TEMP_HOME, ".openclaw", "extensions", "memoree");
    expect(existsSync(pluginDir)).toBe(true);
    expect(existsSync(join(pluginDir, "dist", "index.js"))).toBe(true);
    expect(existsSync(join(pluginDir, ".memoree_version"))).toBe(true);
  });
  ...
  describe("openclaw.json patching (issue #121)", () => {
    ...
    it("adds 'memoree' to tools.alsoAllow when missing", async () => {
      const configPath = writeConfig({
    ...
    it("patches BOTH arrays in a single run when both miss memoree", async () => {
      const configPath = writeConfig({
    ...
    it("is idempotent — no patch when both arrays already include memoree", async () => {
      const configPath = writeConfig({
        plugins: { allow: ["memory-wiki", "memoree"] },
        tools: { profile: "coding", alsoAllow: ["memoree"] },
      });
      ...
      const backups = readdirSync(join(TEMP_HOME, ".openclaw")).filter(n => n.startsWith("openclaw.json.bak-memoree-"));
    ...
    it("does not duplicate 'memoree' if it sneaks in twice", async () => {
      ...
      expect(allow.filter(x => x === "memoree").length).toBe(1);
      expect(also.filter(x => x === "memoree").length).toBe(1);
    });
  ...
  it("warns when embed-deps node_modules is missing instead of silently skipping", async () => {
    ...
    const err = logs.join("");
    const embedDepsNm = join(TEMP_HOME, ".memoree", "embed-deps", "node_modules");
    const expected =
  ...
  it("symlinks embed-deps node_modules into the plugin dir when present", async () => {
    ...
    const embedNm = join(TEMP_HOME, ".memoree", "embed-deps", "node_modules");
    ...
    const pluginNm = join(TEMP_HOME, ".openclaw", "extensions", "memoree", "node_modules");
