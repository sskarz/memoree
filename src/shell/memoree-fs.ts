...
function resolveEmbedDaemonPath(): string {
  // This module is bundled to `<agent>/bundle/shell/memoree-shell.js`,
...
//   2. This file, so the standalone memoree-shell exposes the same VFS to
...
export class MemoreeFs implements IFileSystem {
    ...
    const memoryBootstrap = (async () => {
      ...
      try {
        ...
        for (const row of rows) {
          ...
          // memoree_goals / memoree_kpis tables. Pre-routing hook
          ...
          // but absent from `memoree goal list` (the CLI reads only the
    ...
    const sessionsBootstrap = (sessionsTable && sessionSyncOk) ? (async () => {
      try {
        const sessionRows = await client.query(
          // NOTE: SUM(size_bytes) returns NULL on the Memoree backend when combined
   ...
   *   - Pros: 1 row per goal makes the Memoree table view obvious,
   *     no row proliferation, simpler bootstrap queries.
   *   - Cons: no audit trail; vulnerable to Memoree's
