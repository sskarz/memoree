...
/**
 * Antigravity first-invocation async setup:
...
const log = (msg: string) => _log("agy-session-setup", msg);

const { log: wikiLog } = makeWikiLogger(join(homedir(), ".gemini", "hooks"));
...
async function createPlaceholder(api: StorageBackend, table: string, sessionId: string, cwd: string, userName: string, orgName: string, workspaceId: string): Promise<void> {
  await createPlaceholderSummary(
    (sql) => api.query(sql),
    { table, sessionId, cwd, userName, orgName, workspaceId, agent: "antigravity", pluginVersion: PLUGIN_VERSION, dialect: api.dialect },
    wikiLog,
  );
}
